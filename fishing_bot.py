...
Auto fishing (วนลูป):
  ...
  (เหวี่ยงแล้วมินิเกมไม่ขึ้นใน 30 วิ -> เหวี่ยงเบ็ดใหม่)
...
RECAST_CLICKS = 1   # ตอนเหวี่ยงใหม่คลิกกี่ครั้ง (1 = เหวี่ยงใหม่เลย, 2 = คลิกแรกเก็บสาย คลิกสองเหวี่ยง)
...
def recast(game):
    ...
    elif RECAST_CLICKS > 1:
        log(f"รอเกิน {CAST_TIMEOUT} วิ มินิเกมไม่ขึ้น -> เก็บสายแล้วเหวี่ยงใหม่")
    else:
        log(f"รอเกิน {CAST_TIMEOUT} วิ มินิเกมไม่ขึ้น -> เหวี่ยงเบ็ดใหม่")
    ctrl.recasts += 1
